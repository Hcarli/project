...
def compare():
    if request.method == "GET":
        ...
        return render_template("compare.html", countries=sorted_countries)
    ...
    if request.method == "POST":
        ...
        return render_template('compare.html', fig1=fig1_html,fig2=fig2_html, fig3=fig3_html, fig4=fig4_html, \
                               fig5=fig5_html, fig6=fig6_html, fig7=fig7_html,fig8=fig8_html, countries=sorted_countries)


@app.route ("/sollow", methods = ["GET"])
@login_required
def sollow():
    return render_template("sollow.html")


@app.route("/sollowvar", methods = ["GET", "POST"])
@login_required
def change_data():
    if request.method == "GET":
        data = db.execute('SELECT *, countrycode, country FROM pennworld JOIN countries ON country_id = id')
        ...
        return render_template("sollow_var.html", countries=sorted_countries)
    ...
    if request.method == "POST":
        ...
        return render_template("sollow_var.html", countries=sorted_countries, fig1=fig1_html, fig2=fig2_html, fig3=fig3_html, \
...
def analisys():
    ...
    if request.method == "GET":
        ...
        year = request.args.get("year", default="2019")  # Use args for GET method, default to 2019
        ...
        years = sorted(set(entry['year'] for entry in sollow), reverse=True)

        print(sollow)

        sollow = sorted(sollow, key=lambda x: x['y_star'])  # Sort sollow by y_star in ascending order

    return render_template("analysis.html", sollow=sollow,years=years, default_year=year)
...
def analysis2():
    if request.method == "GET":
        ...
        countries = sorted(set(entry['country'] for entry in sollow))
        years = sorted(set(entry['year'] for entry in sollow), reverse=True)

    return render_template("analysis2.html", sollow=sollow, countries=countries, years=years, default_year=year)
